...
def toilet_filter(version, room_size, second_tier, source):
    ...
    if version == "set1":
        ...
        no_count_object_categories = ["window", "door"]
        freq_threshold = 1000
        ...
        def room_criteria(room, house):
            ...
            if not os.path.isfile(f"./data/suncg_data/room/{room.house_id}/{room.modelId}f.obj"):
                return False
            if not os.path.isfile(f"./data/suncg_data/room/{room.house_id}/{room.modelId}w.obj"):
                return False
